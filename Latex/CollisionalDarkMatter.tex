...
In order to test the invariance, we initialize again the system to trigger the Jeans instability with the same parameters as in section \ref{sec: test_jeans} but using different mean velocity $u$.
We ran simulations with $\tau$ = $\{0,\sigma, 2\sigma\}$.
The perturbation size of the perturbation $\delta(x)$ is quantified by:
\begin{equation}
...
We plotted the evolution of $A_2(t)/A_2(0)$ for every run of the simulation in figure \ref{fig: Galilean_test}.
We observe that regardless of the value of $u$ the behaviour of the Fourier coefficient is the same, thus testing successfully Galilean invariance.
