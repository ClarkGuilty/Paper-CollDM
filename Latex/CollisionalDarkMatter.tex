...
\begin{equation}
f(x,v,t = 0) = \frac{\bar{\rho}}{(2\pi \sigma^2)^{1/2}} \exp(-\frac{(v-u)^2}{2 \sigma^2}) (1 + A\cos(kx)).
\end{equation} 
Where $A$ and $k$ are respectively the amplitude and frequency of the perturbation, $u$ is the mean velocity, and $\sigma$ is the velocity dispersion.
...
T = (G \bar{\rho})^{-1/2},
\end{equation}
and it relates to the phase space domain by $V = L / T$. Without loss of generality, we fix $L = T = 1$, thus $V = 1$. 
Additionally, we fix the mean velocity $u = 0$.
...
%TODO: el test de Jeans per se. Ser\'ia mirar que la taza de crecimiento lineal (o de atenuamiento) sea paralela a la evoluci\'on de $A_2(t)/A_2(0)$.

\subsection{Test III: Galilean invariance}
Given the symplectic and Lagragian nature of the method, we expect it to assure Galilean invariance.
To test it, we initialize the system in a Jeans instability with the same parameters as in section \ref{sec: test_jeans} but using different mean velocity $u$.
We ran simulations with $u$ = $\{0,\sigma, 2\sigma\}$, for $k/k_j = 0.5$ and $k/k_j = 1.1$.
However, due the boundary conditions on the velocity space we must reduce $\sigma$ in order to keep the %simulation stable in the run with $k/k_j = 1.1$.
Due the periodicity of the perturbation, its size $\delta(x)$ is quantified by:
\begin{equation}
\delta(x) = \frac{\rho(x) -\bar{\rho}}{\bar{\rho}},
\end{equation}
and in terms of its Fourier power series:
\begin{equation}
\delta(x) = \sum_{m\geq0} A_m \exp(2i\pi m  x/L).
\end{equation}
Since we chose $n=2$ in $k$, the evolution of the second Fourier coefficient ($A_2$) will quantify the evolution of the initial perturbation, regardless of the phase of the wave.
We observe for both $k/k_j = 0.5$ (figure \ref{fig: Galilean_test1}) and $k/k_j = 1.1$ (figure \ref{fig: Galilean_test2}) that regardless of the value of $u$ the evolution of the Fourier coefficient is the same, therefore the behavior of the perturbation is independent of the mean velocity of the system.
Thus, successfully testing Galilean invariance.


\begin{figure}
    \includegraphics[scale=0.5]{images/gauss.png} %TODO cambiar color algo que funcione a escala de grices.
...
\begin{figure}
    \includegraphics[scale=0.5]{images/Jeans2Coef.png} %TODO cambiar color algo que funcione a escala de grices.
    %\caption{Numerical test of Landau dampening for Gaussian initial conditions. We ran simulations with different values of $\tau$. All units are simulation units and have not being scaled for any astrophysical system in particular. It is easy to observe the thin arm structure mentioned in section \ref{sec: test_gauss}}.
    \label{fig: Galilean_test1}
\end{figure}
\section{Conclusions}
